#!/usr/bin/python3
import heapq
class infromedsearch:
    def __init__(self, graph, heuristic):
        self.graph = graph
        self.heuristic = heuristic
        
    def astar(self, start, goal, path):
        pri_queue = []
        visited = set()
        # heapq.heapush(list, (f(h), g(h), start, path))
        heapq.heappush(pri_queue, (self.heuristic.get(start), 0, start, path))
        
        while pri_queue:
            
            f, g, current_city, path = heapq.heappop(pri_queue)
            if current_city == goal:
                return f, path + [current_city]
            new_path = path + [current_city]
            
            if current_city not in visited:
                visited.add(current_city)
                
            for neighbor, cost in self.graph.get(current_city, []):
                heapq.heappush(pri_queue, (self.heuristic.get(neighbor) + cost + g, cost + g, neighbor, new_path))
...
problem = infromedsearch(graph, heuristic)
result = problem.astar(start, goal, [])
print(result)
